...
- (void)showDateFunction {
    ...
    NSString *dayText = [[YXDateHelpObject manager] getStrFromDateFormat:@"d" Date:_cellDate];
    NSString *monthText = [NSString stringWithFormat:@"%@月", [[YXDateHelpObject manager] getStrFromDateFormat:@"MM" Date:_cellDate]];
    _dayL.text = dayText;
    
    if ([[YXDateHelpObject manager] isSameDate:_cellDate AnotherDate:[NSDate date]]) {
        _dayL.layer.borderWidth = 1.5;
        _dayL.layer.borderColor = [UIColor colorWithHexString:@"#006db8"].CGColor;

        NSString *dayAndMonthText = [NSString stringWithFormat:@"%@\n%@", monthText, dayText];
        _dayL.text = dayAndMonthText;
        NSMutableAttributedString *attString = [[NSMutableAttributedString alloc] initWithString:dayAndMonthText];
        NSRange range = NSMakeRange(0, monthText.length);
        [attString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:10] range:range];
        [_dayL setAttributedText:attString];
        _dayL.numberOfLines = 0;
    ...
    if (_selectDate) {
        ...
        if ([[YXDateHelpObject manager] isSameDate:_cellDate AnotherDate:_selectDate]) {
            _dayL.backgroundColor = [UIColor colorWithHexString:@"#006db8"];
